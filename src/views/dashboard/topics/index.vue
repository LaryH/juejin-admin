<template>
	<a-card>
		<!-- 添加 -->
		<div>
			<a-button type="primary" @click="showAdd" style="    margin-bottom: 15px"
				>+ 添 加</a-button
			>
			<a-modal
				title="添加品牌"
				...
				>
					<a-form-item label="话题名称">
						<a-input
							v-decorator="[
								'note',
								{ rules: [{ required: true, message: '请添加你的话题名称!' }] }
							]"
					...
					<a-form-item label="上传图片">
						...
						<a-upload
							...
							v-decorator="[
								'gender',
								{ rules: [{ required: true, message: '请上传你的图片!' }] }
							]"
						>
							<img
								v-if="brand.topic.icon"
								:src="brand.topic.icon"
								alt="avatar"
							/>
							<div v-else>
		...
		>
			<template
				v-for="col in ['topic.topic_id', 'topic.brandTitle']"
				slot-scope="text"
			>
				<div :key="col">
			...
			<template slot="operation" slot-scope="text, record">
				...
				<a-button
					...
					>修改</a-button
				>
				<a-button type="danger" :size="size" @click="del(record)"
					>删 除</a-button
				>
			</template>
		</a-table>
